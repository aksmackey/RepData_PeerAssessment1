...
```
### The mean number of steps taken per day is `r meanstp` and the median is `r medstp`.  
...
```
### The highest average number of steps for any time interval was `r maxstep` during the `r maxint` interval.  
...
```
### The total number of NAs in the steps data is `r nacount`.
...
```
### The new mean number of steps taken per day is `r meanstp2` and the new median is `r medstp2`. The mean did not change, but the median went up by one step. Imputing the NA values had little to no effect on the data. 
